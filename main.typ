...
#show: template.with(
  ...
  resumo-conteudo: [
    Este trabalho apresenta o Centro de Ajuda, uma prova de conceito destinada a facilitar a identificação dos serviços adequados para o atendimento de situações emergenciais. A proposta parte da fragmentação dos números de emergência no Brasil e das dificuldades que essa organização pode gerar, especialmente em momentos de medo, estresse ou urgência. O objetivo é permitir que o usuário descreva uma ocorrência e receba uma indicação de atendimento compatível com sua necessidade. Para isso, foi desenvolvida uma solução organizada em aplicação cliente e servidor, com inteligência artificial para interpretar relatos em texto, áudio e imagem, considerando informações de localização e horário. A avaliação comparou sete modelos em quatro cenários, com uma execução por combinação de modelo e cenário, observando compatibilidade com as entradas e adequação das respostas. Nas condições testadas, apenas o Gemini 3.5 Flash processou diretamente todas as modalidades nos quatro cenários. Os demais apresentaram limitações que motivaram uma análise complementar da utilização conjunta de modelos especializados. Os resultados indicam potencial técnico para apoiar o direcionamento de solicitações, mas a ausência de repetições limita sua generalização. Conclui-se que a proposta oferece uma base para novos estudos e aperfeiçoamentos, dependendo de avaliações mais amplas, validação dos contatos e melhorias de confiabilidade antes de uma eventual utilização em situações reais, sem substituir as centrais oficiais.
  ],
  resumo-palavras: ("Inteligência Artificial", "Emergência", "Multimodal", "LLM"),
  abstract-conteudo: [
    This study presents Centro de Ajuda, a proof of concept designed to help users identify appropriate services for emergency situations. The proposal addresses the fragmentation of emergency telephone numbers in Brazil and the difficulties this arrangement may create, particularly in moments of fear, stress, or urgency. Its objective is to allow users to describe an incident and receive a service recommendation suited to their needs. To this end, a solution comprising a client application and a server was developed, using artificial intelligence to interpret text, audio, and image reports alongside location and time information. The evaluation compared seven models across four scenarios, with one execution per model–scenario combination, examining input compatibility and response appropriateness. Under the tested conditions, only Gemini 3.5 Flash directly processed all input modalities across the four scenarios. The remaining models showed limitations that motivated a complementary analysis of combining specialized models. The findings suggest technical potential for supporting emergency service selection, although the lack of repeated trials limits their generalizability. The study concludes that the proposal provides a foundation for further research and development, requiring broader evaluation, verification of contact information, and improvements in reliability before potential use in real situations, without replacing official emergency dispatch centers.
  ],
    ...
    #align(center)[
      #block(width: 13.5cm)[
        ...
        #rect(
          ...
          Cabral, Pedro Victor Hipólito

          Centro de Ajuda: Sistema Inteligente de
          Redirecionamento de Comunicação em Situações
          Emergenciais / Pedro Victor Hipólito Cabral. —
          Santo Antônio de Jesus, Bahia, 2026.

          [TOTAL DE PÁGINAS] p.: il.
          ...
          Tecnologia da Bahia, campus Santo Antônio
          de Jesus, 2026.
          ...
          I. Souza, Leandro Costa.
          II. Instituto Federal de Educação, Ciência
          e Tecnologia da Bahia, campus Santo Antônio
          de Jesus.
          III. Título.

          #align(right)[CDD: [A DEFINIR]]
        ]
      ]
    ]
...
#codigo(
  lang: "javascript",
  caption: [Configuração do Express],
  filename: "express.js",
...
#codigo(
  lang: "javascript",
  caption: [Tratamento das solicitações ao servidor],
  filename: "handleendpoint.js",
...
#codigo(
  lang: "javascript",
  caption: [Conversão de arquivos para Base64],
...
#figura(
  image("assets/imagens/arquitetura-geral.png", width: 90%),
  caption: [Arquitetura geral do sistema],
) <fig:arquitetura-geral>
...
#codigo(
  ...
  read("assets/codigos/systemprompt.js"),
)
...
#codigo(
  lang: "javascript",
  caption: [Assistant Prompt contextual],
  filename: "assistantprompt.js",
  read("assets/codigos/assistantprompt.js"),
)
...
#codigo(
  lang: "javascript",
  caption: [Construção do conteúdo multimodal],
  filename: "buildcontent.js",
  read("assets/codigos/buildcontent.js"),
)
...
#codigo(
  lang: "javascript",
  caption: [Envio da requisição ao OpenRouter],
  filename: "requestopenrouter.js",
  read("assets/codigos/requestopenrouter.js"),
)
...
#codigo(
  lang: "javascript",
  caption: [Registro da resposta e das métricas da requisição],
  filename: "openrouteres.js",
  read("assets/codigos/openrouteres.js"),
)
...
#codigo(
  lang: "javascript",
  caption: [Validação da estrutura da resposta],
  filename: "validateopenrouter.js",
  read("assets/codigos/validateopenrouter.js"),
)
...
#tabela(
  caption: [Comparação por critérios — modelo isolado],
  columns: (2fr, 1fr, 1fr, 1fr, 1fr, 1fr, 1fr, 1fr),
  align: (left, center),
  font-size: 10pt,
  header: ([Critério], [GPT], [Claude], [Llama], [Qwen], [Gemini], [Nemotron], [North]),
...
#tabela(
  caption: [Viabilidade por ecossistema (composição sequencial)],
