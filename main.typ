...
#show: template.with(
  ...
  resumo-conteudo: [Este trabalho apresenta o desenvolvimento de uma prova de conceito de um sistema inteligente para redirecionamento de solicitações emergenciais, denominado #emph[Centro de Ajuda], que utiliza inteligência artificial multimodal para interpretar relatos em texto, áudio e imagem e recomendar o serviço de emergência mais adequado.],
  resumo-palavras: ("Inteligência Artificial", "Emergência", "Multimodal", "LLM"),
  abstract-conteudo: [This work presents a proof of concept of an intelligent emergency redirection system called #emph[Centro de Ajuda], using multimodal AI to interpret text, audio and image reports and recommend the most appropriate emergency service.],
...
O GPT-5.5, desenvolvido pela OpenAI, foi selecionado para análise devido à sua relevância no cenário atual da inteligência artificial generativa. A avaliação do modelo foi realizada com base nos critérios técnicos definidos para a seleção das #abbrev("LLMs") utilizadas na proposta deste trabalho. \cite{GPT-5.5}
...
Desenvolvido pela Anthropic, o Claude Opus 4.8 foi incluído na análise por representar uma das principais famílias de modelos de linguagem disponíveis comercialmente. Sua avaliação considerou os critérios técnicos estabelecidos para verificar sua adequação à proposta do sistema #emph[Centro de Ajuda]. \cite{Claude}
...
O Llama 4 Maverick, desenvolvido pela Meta, foi incluído na análise por integrar uma das principais famílias de modelos de linguagem com pesos abertos disponíveis atualmente. Sua avaliação foi conduzida considerando os critérios técnicos definidos para verificar sua adequação aos requisitos do sistema #emph[Centro de Ajuda]. \cite{Llama 4}
...
O Qwen 3.6 Plus, desenvolvido pela Alibaba Cloud, foi incluído nesta análise por representar uma das principais soluções proprietárias da família Qwen. Sua avaliação teve como objetivo verificar o atendimento aos critérios técnicos definidos para a seleção das #abbrev("LLMs") empregadas no sistema #emph[Centro de Ajuda]. \cite{QwenAPI}
...
O Gemini 3.5 Flash, desenvolvido pela Google, foi selecionado para análise em razão de sua relevância entre os modelos multimodais atuais e da disponibilidade de acesso por meio de APIs compatíveis com a proposta deste trabalho. Sua avaliação foi conduzida considerando os critérios técnicos estabelecidos para a seleção das 

#abbrev("LLMs") empregadas no sistema #emph[Centro de Ajuda]. \cite{Gemini}
...
O Nemotron 3 Nano Omni 30B A3B Reasoning, desenvolvido pela NVIDIA, foi incluído na análise por representar um modelo multimodal voltado a aplicações que exigem raciocínio sobre diferentes tipos de dados. Sua avaliação foi realizada considerando os critérios técnicos definidos para a seleção das #abbrev("LLMs") empregadas na proposta do sistema #emph[Centro de Ajuda]. \cite{Nemotron}
...
O North Mini Code, desenvolvido pela Cohere, participou desta análise por ocupar espaço como um modelo de peso, voltado à geração de código e à execução de tarefas de engenharia de software por agentes. O modelo utiliza uma arquitetura \textit{Mixture of Experts}, com 30 bilhões de parâmetros totais e 3 bilhões ativos durante a inferência. Sua avaliação foi realizada considerando os critérios técnicos definidos para verificar sua adequação à proposta do sistema #emph[Centro de Ajuda]. \cite{NorthMiniCode}
...
O backend concentra o processamento necessário para transformar os dados enviados pelo usuário em uma resposta utilizável pela aplicação. Para viabilizar a prova de conceito do sistema #emph[Centro de Ajuda], essa camada precisava cumprir quatro funções principais: receber solicitações contendo diferentes modalidades de entrada, complementar os dados com informações de localização, encaminhar o contexto da ocorrência ao modelo de linguagem e devolver ao frontend uma resposta estruturada. 
...
Neste capítulo são apresentados os detalhes de desenvolvimento do sistema #emph[Centro de Ajuda]. São descritas as decisões de projeto, as tecnologias utilizadas, o funcionamento do aplicativo cliente (frontend) e do servidor (backend), a construção dos prompts enviados aos modelos de linguagem e o processo de avaliação por meio do benchmark. O objetivo deste capítulo é fornecer uma visão completa de como a solução foi implementada e como os componentes interagem para entregar ao usuário a recomendação do serviço de emergência mais adequado à situação descrita.
...
Conforme a Figura~\ref{fig:diagrama_desenvolvimento_frontend_arquitetura}, esses componentes foram organizados de maneira sequencial, acompanhando as etapas realizadas desde o registro da ocorrência até a apresentação da recomendação ao usuário.
...
Após o envio da solicitação, a aplicação redireciona o usuário para uma tela intermediária, ilustrada na Figura~\ref{fig:diagrama_desenvolvimento}. Essa tela tem como objetivo fornecer um retorno visual imediato de que a requisição foi recebida e está sendo processada pelo servidor, evitando que o usuário interprete a ausência de resposta como uma falha na aplicação.
...
Após o processamento da solicitação, a aplicação apresenta ao usuário a tela de resposta, ilustrada na Figura~\ref{fig:diagrama_desenvolvimento_frontend_resultado}. Essa interface foi projetada para que destaque as informações mais relevantes da recomendação gerada, permitindo que o usuário identifique rapidamente qual serviço de emergência deve ser acionado.
...
Com o desenvolvimento da prova de conceito, tornou-se necessário avaliar como diferentes modelos de linguagem se comportam quando integrados à arquitetura proposta. Essa etapa teve como finalidade verificar não apenas a capacidade dos modelos de produzir respostas, mas também sua adequação às necessidades do #emph[Centro de Ajuda], especialmente diante das diferentes formas de entrada previstas pelo sistema.
...
O desenvolvimento do #emph[Centro de Ajuda] também ampliou a compreensão sobre o funcionamento de modelos multimodais em aplicações integradas por API. Durante a implementação e a realização dos testes, foi possível identificar que a multimodalidade apresentada nas interfaces de conversação dos diferentes ecossistemas nem sempre corresponde às capacidades disponíveis diretamente em seus modelos ou endpoints.
...
O #emph[Centro de Ajuda] demonstrou viabilidade técnica como apoio ao direcionamento emergencial, integrando entrada multimodal, geocodificação, prompts e OpenRouter para entregar `service_name`/`phone_number`/`emergency_context` com redirecionamento `tel:`.
...
Embora os resultados obtidos tenham demonstrado a viabilidade da proposta, o #emph[Centro de Ajuda] ainda possui possibilidades de aprimoramento antes de uma eventual utilização em situações reais. As propostas apresentadas a seguir envolvem a evolução técnica da aplicação, a ampliação do processo de avaliação, a investigação de outros modelos de linguagem e o desenvolvimento de mecanismos capazes de aumentar a confiabilidade das recomendações produzidas.
...
Os resultados alcançados representam, portanto, uma etapa inicial do desenvolvimento da proposta. As possibilidades apresentadas para sua continuidade demonstram que o #emph[Centro de Ajuda] ainda possui um amplo campo de evolução, tanto em sua implementação quanto na avaliação e na confiabilidade das recomendações. Assim, este trabalho estabelece uma base para novos estudos e aperfeiçoamentos que poderão aproximar a proposta de uma aplicação mais segura, abrangente e adequada a situações reais.
